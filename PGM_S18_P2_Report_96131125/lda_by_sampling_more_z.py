def lda_by_sampling_more_z(samples, W, T, alpha, beta, max_epoch, back_toback_z=True):
    """
    lda by gibbs sampling use more than one z
    ...
    :param back_toback_z: use z1, z2,...,z10 for calculating phi and theta otherwise, use z1, z5, z10, z15, .. z45
    :return:
    """
    ...
    for i_iter in range(0, max_epoch):
        ...
        phi_more_z = np.zeros(shape=(T, W))
        theta_more_z = np.zeros(shape=(D, T))

        if back_toback_z == True:
            for i_th in range(0, len(phis)):
                phi_more_z = phi_more_z + phis[i_th]
                theta_more_z = theta_more_z + thetas[i_th]
            phi_more_z = phi_more_z / len(phis)
            theta_more_z = theta_more_z / len(thetas)
        else:
            count_ = 0
            if len(phis) == 1:
                count_ = 1
                phi_more_z = phis[0]
                theta_more_z = thetas[0]

            for i_th in range(len(phis)-1, 0, -5):
                count_ += 1
                phi_more_z = phi_more_z + phis[i_th]
                theta_more_z = theta_more_z + thetas[i_th]
            phi_more_z = phi_more_z / count_
            theta_more_z = theta_more_z / count_

        perplexity_epoch_i = perplexity(samples=samples, phi=phi_more_z, theta=theta_more_z)
        perplexities.append(perplexity_epoch_i)

    return phi_more_z, theta_more_z, perplexities, time_total
