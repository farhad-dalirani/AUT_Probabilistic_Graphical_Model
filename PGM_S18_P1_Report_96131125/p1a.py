...
img_gaussian_noise = add_noise(img=img_noise_free_gl, mean=0, variance=0.01)
    ...
    for j in range(0, img_gaussian_noise.shape[1]):
        ...
        for key in priors:
            probabilities[key] = priors[key] * ((1/(np.sqrt(2*np.pi)*conditional_p[key]['std'])) *
                                                (np.exp(((img_gaussian_noise[i, j]-conditional_p[key]['mean'])**2) /
...
plt.title('Input After Adding Gaussian Noise (0, 0.01)')
plt.figure()
plt.imshow(segments, cmap='gray')
plt.title('Segmented image')

mpimg.imsave('output/p1a-1.png', img_gaussian_noise, cmap='gray')
mpimg.imsave('output/p1a-2.png', segments, cmap='gray')
plt.show()